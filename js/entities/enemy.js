...
const BOSS_SCHEDULE = [
  ...
  { base: "juggernaut", bossTitle: "WARLORD GRIM", label: "Warlord Grim",
    speed: 100, damage: 55, maxHealth: 1500, slamRadius: 110, attackCooldown: 0.8,
  ...
  { base: "broodmother", bossTitle: "QUEEN VESPERA", label: "Queen Vespera",
    speed: 72, damage: 24, maxHealth: 1300, burstCount: 9, burstSpread: 0.9,
    projectileSpeed: 620, attackCooldown: 0.55, radius: 34, score: 850,
  ...
  { base: "titan", bossTitle: "THE MONOLITH", label: "Monolith",
    speed: 110, damage: 70, maxHealth: 2200, slamRadius: 140,
    shockwaveCount: 18, attackCooldown: 0.7, radius: 48, score: 1100,
    bodyColor: "#2a0d3a", accentColor: "#aa55ee", bossGlowColor: "#bb44ff",
    chaseAttackInterval: 1.2, eyeColor: "#eeccff" },
  // Boss #7 (wave 35) — hybrid: ranged burst + melee slam + shockwave
  { base: "broodmother", bossTitle: "THE AMALGAM", label: "Amalgam",
    speed: 90, damage: 30, maxHealth: 2800, burstCount: 8, burstSpread: 0.8,
    projectileSpeed: 580, attackCooldown: 0.5, radius: 40, score: 1400,
    bodyColor: "#4a2010", accentColor: "#ff8833", bossGlowColor: "#ff6600",
    projectileColor: "#ffaa44", slamRadius: 100, shockwaveCount: 8,
    dualMode: true, eyeColor: "#ffcc66" },
  // Boss #8 (wave 40) — ultimate melee juggernaut
  { base: "titan", bossTitle: "OMEGA", label: "Omega",
    speed: 115, damage: 70, maxHealth: 3800, slamRadius: 150,
    shockwaveCount: 20, attackCooldown: 0.45, radius: 50, score: 2000,
    bodyColor: "#1a0a0a", accentColor: "#ff2222", bossGlowColor: "#ff1111",
    eyeColor: "#ff4444", chaseAttackInterval: 1.0 },
];
...
export function getBossConfigForOccurrence(n) {
  ...
  if (n >= BOSS_SCHEDULE.length) {
    ...
    config.maxHealth = Math.floor(config.maxHealth * (1 + extra * 0.35));
    config.damage = Math.floor(config.damage * (1 + extra * 0.2));
    config.speed = Math.floor(config.speed * (1 + extra * 0.06));
...
let ENEMY_TYPES = {
  ...
  sprinter: {
    ...
    attackCooldown: 0.65,
    noticeRange: 440,
    retreatThreshold: 0.06,
    retreatTime: [0.35, 0.55],
    retreatCooldown: 2.8,
    bodyColor: "#d4783a",
    accentColor: "#ffa54e",
    eyeColor: "#ffe08e",
  ...
  spitter: {
    ...
    damage: 14,
    attackRange: 200,
    preferredDistance: 155,
    attackWindup: 0.5,
    attackRecovery: 0.24,
    attackCooldown: 1.2,
    noticeRange: 450,
    retreatThreshold: 0.65,
    retreatTime: [0.6, 1.0],
    retreatCooldown: 2.4,
    bodyColor: "#5aad58",
    accentColor: "#7bcf78",
...
export class Enemy {
  constructor({ x, y, type, wave }) {
    ...
    this.maxHealth = this.config.maxHealth + wave * 5;
    this.health = this.maxHealth;
    this.speed = this.config.speed * (1 + wave * 0.025);
    this.noticeRange = this.config.noticeRange * (1 + wave * 0.018);
